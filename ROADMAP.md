...
| REQ-84 | P1 | 「导入会话」窗口接入官方原生右侧栏 tab（DSH ≥ 0.1.5-rc.1 的 `sidebarRightTabs` / `sidebar.right.pane.tab`，参照 dsh-context 的接入姿势；footer 按钮打开同一 tab、对话区保留；左侧栏保持单一入口，老版本回落既有入口） | ✅ |
