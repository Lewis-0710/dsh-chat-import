#!/usr/bin/env bash
#==============================================================================
# sync.sh — dsh-chat-import 上游同步脚本
#==============================================================================
#
# 功能：
...
for arg in "$@"; do
    case "$arg" in
        ...
        -h|--help)
            ...
            echo "  --skip-test    跳过 npm/pnpm 测试验证"
            echo "  -h, --help     显示帮助信息"
            exit 0
            ;;
    esac
...
if ! git remote get-url upstream >/dev/null 2>&1; then
    ...
    echo "💡 请先运行: git remote add upstream https://github.com/Nwflower/dsh-chat-import.git"
    exit 1
fi
...
# Step 0: 创建安全临时备份目录
TMP_DIR="$(mktemp -d /tmp/dsh_chat_import_sync_XXXXXX)"
cleanup() {
    rm -rf "$TMP_DIR"
}
