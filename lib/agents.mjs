// lib/agents.mjs — 外部 agent/mode prompt 落盘转换为 DSH skills 资产
//
...
//
// 与 context-bridge.mjs 互补：它是运行时只读桥接（Claude 侧、
...
/**
 * 收集 Claude 资产候选（ctx 驱动层，复用 collectCandidates 的 provenance 语义）：
 ...
 * 与 context-bridge 互补：bridge 是运行时 scoped 注入，本项是持久资产，可同时生效。
 */
...
/**
 * 入口：收集 pi/opencode/Claude/Codex 候选 → 规划 → （preview 时）只返回 plan /
 * （apply 时）落盘。
