...
// 的来源沿旧契约登记进 IMPORT_SPECS（Stage 2），lib/panel.mjs 的
...
// Stage 2：面板批量导入（POST /api-import/import）与 /import 命令按 format
...
// Incremental append semantics (): complements the "idempotent skip" phrasing in the
...
const CHAT_FORMATS = [
  ...
  ['opencode', 'opencode db opencode.db (~/.local/share/opencode)'],
  ...
  ['dsh', 'DSH session logs session[.vN].jsonl[.zstd] ($DSH_HOME/sessions)'],
  ['local-jsonl', 'Any local JSONL (auto-detected; parseFormat forces a parser)'],
]
...
// 参数收敛：全部源共享一份公共参数表（分发器单工具形态，不再按源复制）。
// opencode/mimocode/teleagent/zcode 原 drop 的 sessionId/recursive 由 SQLite 源自然忽略。
const COMMON_PARAMS = {
  path: {
    type: 'string',
    required: true,
...
const EXTRA_PARAMS = {
  compacted: {
    type: 'boolean',
    description: 'Optional (claude only): import just the last compaction summary + tail.',
  },
  branch: {
    type: 'string',
    enum: ['main', 'all'],
  ...
  sessionIds: {
    type: 'array',
    items: { type: 'string' },
    description: 'Optional (opencode/mimocode/teleagent/zcode only): import only these source session ids.',
  },
  fullHistory: {
    type: 'boolean',
    description: 'Optional (opencode/mimocode/pi only): true ignores compaction and imports full history.',
  },
  lineage: {
    type: 'string',
    enum: ['tail'],
  ...
  parseFormat: {
    type: 'string',
    enum: PARSE_FORMATS,
    description: 'Optional (local-jsonl only): force a parser; default auto-detects.',
  },
}
...
export function buildImportExecutor(ctx, spec) {
  ...
  return async (args) => {
    // 解析上下文预算（参数 > env > 动态模型窗口 > 静态默认），盖写进
    ...
    // preview/dryRun=true 走预览分支（照常 resolve/stat/readText/convert，
...
// 描述里的格式数由枚举实时推导：新增来源只改 CHAT_FORMATS，描述不再需要手工同步
//（历史上有过描述写死 20、枚举已 22 的漂移）。local-jsonl 不是「面板来源」，不计入。
...
function renderImportResult(args, value, spec) {
  ...
  // 畸形行明细 + secrets/permission 计数：只含行号与 kind，绝不拼入内容
  const detailsNote = (v) => {
  ...
  return [{
    type: 'text',
    text: '已导入 ' + value.turns + ' 轮对话（' + value.messages + ' 条消息、' + value.toolCalls + ' 次工具调用）→ 会话 ' + value.sessionId + (value.skipped ? '（跳过 ' + value.skipped + ' 行畸形记录）' : '') + trimmedNote(value) + (detailsNote(value) ? '\n' + detailsNote(value) : ''),
  }]
}
  ...
  oneOf: [
      ...
      properties: {
        ...
        cwdRemap: {
          ...
          properties: {
            ...
            mapped: { type: 'string', required: true },
    ...
    // 单文件模式
    {
      ...
      properties: {
        ...
        status: { type: 'string', required: true, enum: ['imported', 'already-imported', 'appended', 'skipped', 'ignored'] },
        // Reasonix WAL 合并 / Claude compacted 摘要导入报告
        ...
        orphanToolResults: { type: 'integer' },
        duplicateToolResults: { type: 'integer' },
        trimmed: {
          type: 'object',
          additionalProperties: false,
          properties: {
          ...
          items: {
            ...
            properties: {
              ...
              status: {
                type: 'string',
                required: true,
                enum: ['imported', 'already-imported', 'appended', 'skipped', 'failed', 'ignored'],
              },
