// lib/opencode.mjs — opencode SQLite 历史库读取与导入编排（拆分自 index.mjs）
//
...
export async function importOpencodeFile(ctx, target, args, { registryDir, persisted, runDecision, markTrimmedSource, filter, convert = convertOpencodeJson, readDb = readOpencodeDb, sourceLabel = 'opencode' } = {}) {
  ...
  const decision = await decideMulti(ctx, { known, items, stat: stat ? { ...stat, walSig } : stat, args, fingerprint, persisted: persistedSet, sourcePath: path, subTable: 'sessions', budget: args.budget, archivedIds, importFormat: 'opencode' })
