// lib/cwd-map.mjs — full：cwd 权威映射 + 沙箱防护（host 面，消费 ctx.fs）
//
...
/**
 * DSH 已知工作区路径集合（<DSH_HOME>/profiles/<profile>/workspace.json + workspaceRegistry，
 * 带缓存）。导出给发现层用：有些来源的会话库就放在**项目目录里**（如 Crush 的
 * <项目>/.crush/crush.db），只能靠「这个用户有哪些工作区」去逐个探测。
 */
export async function knownWorkspacePaths(ctx) {
  const now = Date.now()
