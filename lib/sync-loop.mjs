...
import { loadImports, rememberImport, unwrapRecord, archivedSessionIds, findSourcePathByDshId, listPersistedHeaders, listPersistedIds, readSessionRecord, canReadSessionEvents } from './imports.mjs'
...
import { serializeClaudeJsonl, serializeClaudeJsonlTail, tailClaudeEvents, verifyClaudeJsonl, slugifyClaudeCwd } from './export/index.mjs'
...
import { convertClaudeJsonl, convertCodexJsonl, convertGrokbuildJson } from './convert/index.mjs'
import {
...
async function outboundOne(ctx, registryDir, header, targetFormat, dryRun, roots = {}) {
  const sessionRecord = await readSessionRecord(ctx, header.id, 0)
  if (!sessionRecord) throw new Error('会话日志读不到：' + header.id)
