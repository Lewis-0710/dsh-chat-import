...
import { slugifyClaudeCwd, serializeClaudeJsonl, serializeBundle, serializeCodexJsonl, serializeKimiWire, serializeOpencodeJson } from '../export.mjs'
import { exportDegradations } from '../convert.mjs'
import { loadImports, rememberImport, unwrapRecord, findSourcePathByDshId, listPersistedHeaders, readSessionRecord, canReadSessionEvents } from './imports.mjs'
...
export async function exportClaudeSession(ctx, args, { uuid = randomUUID, registryDir } = {}) {
  ...
  const sessionRecord = await readSessionRecord(ctx, args.sessionId, 0)
  if (!sessionRecord) throw new Error('会话日志读不到：' + args.sessionId)
...
// REQ-23 矩阵化互转通用导出执行体（export_codex / export_kimi / export_opencode）：
// DSH 会话 → 目标格式文件（serialize 注入），只读会话日志、写盘 createIfAbsent、dryRun
// 不写盘、降级逐条报告（REQ-21）。与 export_claude 同构，但目标不是 Claude
//（无 sync 写回副本，不落 registry exports）。field 指序列化器里承载文件正文的键
//（行式格式是 jsonl，opencode 是单个 JSON 文档 json）。
async function exportTargetFile(ctx, args, { serialize, ext, field = 'jsonl' }) {
  const sp = ctx.get('sessionPersistence')
  if (!sp || typeof sp.list !== 'function' || !canReadSessionEvents(sp)) {
    throw new Error('sessionPersistence 不可用（需要 list + 读事件面）')
  }
  ...
  if (!sessionRecord) throw new Error('会话日志读不到：' + args.sessionId)
  const { meta, events } = sessionRecord
  const titleEvent = Array.isArray(events)
...
// REQ-79：DSH 会话 → opencode `import <file>` 可读的 JSON（写 .opencode.json）。
