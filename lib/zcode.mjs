// lib/zcode.mjs — zcode（z.ai 官方 CLI）SQLite 历史库读取与导入编排
//
...
//
// runDecision / markTrimmedSource 是 claude/chatgpt 等路径共用的共享函数
// （lib/import-core.mjs / lib/budget.mjs，非 zcode 专属），由 lib/tools.mjs 注册工具
// 时经 options 注入（与 importOpencodeFile 同款，见 lib/opencode.mjs 文件头）。
...
// 恒返回批量形态。：DB 级 version/size 短路径检测；逐会话判增 append /
...
export async function importZcodeFile(ctx, target, args, { registryDir, persisted, runDecision, markTrimmedSource } = {}) {
  ...
  if (known && args.force !== true) {
    // 选择性导入守卫：args.zcodeId / zcode://<id> / args.sessionIds 指定的会话不在
    // 已知子表中时短路径不成立——否则按 id 选择性重导会被 DB 指纹短路径判为
    // already-imported，漏导新选中的会话（面板「部分」条目重导无效）。
    const zcodeIdEarly = typeof args.zcodeId === 'string' && args.zcodeId
      ? args.zcodeId
      : isPseudo ? rawPath.slice('zcode://'.length) : undefined
    const guardIds = zcodeIdEarly ? [zcodeIdEarly]
      : (Array.isArray(args.sessionIds) && args.sessionIds.length > 0 ? args.sessionIds : null)
  ...
  for (const s of sessions) {
    ...
    // finalizeConvertedSession 统一钉「来源 · 话题」标题 + cwd 重映射口径对齐
    const out = finalizeConvertedSession(
      markTrimmedSource(convertZcodeJson(JSON.stringify(s), { ...args, sourcePath: path }), args),
      args,
      'zcode',
    )
  ...
  const decision = await decideMulti(ctx, { known, items, stat: stat ? { ...stat, walSig } : stat, args, fingerprint, persisted: persistedSet, sourcePath: path, subTable: 'sessions', budget: args.budget, archivedIds, importFormat: 'zcode' })
