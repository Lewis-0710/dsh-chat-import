...
// 逐会话记录 { dshId, turns, events }。budget 为 上下文预算（token 数），
...
/** dshId → 导入归属 sourcePath 反查（single + multi 子表）。0.8.3 起日志
    ...
    if (record.kind === 'multi') {
      for (const table of ['conversations', 'sessions']) {
        ...
        for (const [subKey, sub] of Object.entries(subs)) {
          if (sub && typeof sub === 'object' && sub.dshId === dshId) return { sourcePath, subTable: table, subKey }
        }
      }
    } else if (record.dshId === dshId) {
      return { sourcePath, subTable: undefined, subKey: undefined }
    }
  }
  return null
}
...
// ── 宿主持久化 API 适配 ──────────────────────────────────────────────────────
// dsh 0.1.5 起 sessionPersistence 换成 per-session 句柄式，插件用到的三个面都变了：
//   list()        → [{ header, revision, sizeBytes }]（旧宿主为 header 数组）
//   readFrom(id)  → 移除，改 open(id, 'read') → handle.read(offset, length) → { events }
//   inspect(id)   → 移除，同上
//   create(meta)  → 返回写句柄（旧宿主无返回值），append(id, events) 移除，
//                   续写改走 handle.append(events)
// 只在本层认新旧两套形状，其余模块继续用「列会话 / 读事件 / 建会话」的语义。
...
/** 现有会话的导入归属路径：imports registry 反查优先（0.8.3 起日志不再写
...
async function decideItem(ctx, { known, converted, args, persisted, sourcePath, archivedIds, importFormat }) {
  ...
  const base = {
    ...
    // 裁剪上报随结果透出（budget/估算/裁剪计数，index 层已并入 source）
    ...(converted.trimmed ? { trimmed: converted.trimmed } : {}),
    // Reasonix WAL 合并 / Claude compacted 摘要导入报告透出
...
const itemRecord = (dshId, t, ev, format) => ({ dshId, turns: t, events: ev, ...(format ? { format } : {}) })
  ...
  // registry 有记录、宿主里却没有这条会话（日志被手工删除 / 清理过）→ 不能按 already-imported
  // 静默跳过，当未导入重建；并在结果里点名（失败要大声，避免留下「registry 说有、磁盘上没有」的幽灵）。
  let staleRegistry = null
  if (known && (!persisted.has(known.dshId) || archived.has(known.dshId))) {
    if (!persisted.has(known.dshId) && typeof known.dshId === 'string') staleRegistry = { previous: known.dshId, reason: 'session-log-missing' }
    known = null
  }
  // 结果里点名（decide* 的公开结果经 runDecision 透传，__ 前缀之外的键原样保留）
  if (staleRegistry) base.staleRegistry = staleRegistry
...
export async function decideMulti(ctx, { known, items, stat, args, fingerprint, persisted, sourcePath, subTable, budget, archivedIds, importFormat }) {
  ...
  // 否则先选 A 再选 B 会让 A 从 imports.json 消失，历史/撤回/增量同步无法定位。
  ...
  const parentRecord = {
    ...
    mtimeMs: stat && typeof stat.mtimeMs === 'number' ? stat.mtimeMs : undefined,
    version: stat && typeof stat.version === 'string' ? stat.version : undefined,
    // WAL 边车签名（见 sqliteWalSig）：S3 短路径除主文件指纹外还要求它一致，
...
// SQLite 库的 WAL 边车签名（S3 短路径指纹补充）：WAL 模式下新写入只落 <db>-wal，
