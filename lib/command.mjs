...
//
// 斜杠命令 `/import <source> <path>`（用户触发、不占模型轮次）：解析 source（FORMATS 里的
...
const TOOL_FORMAT = {
  ...
  antigravity: 'antigravity', import_antigravity: 'antigravity',
  ...
  teleagent: 'teleagent', import_teleagent: 'teleagent',
  ...
  continue: 'continue', import_continue: 'continue',
  cline: 'cline', import_cline: 'cline',
  goose: 'goose', import_goose: 'goose',
  zed: 'zed', import_zed: 'zed',
  crush: 'crush', import_crush: 'crush',
  dsh: 'dsh', import_dsh: 'dsh',
}

const SOURCE_NAMES = 'claude/codex/chatgpt/cursor/gemini/reasonix/opencode/mimocode/teleagent/zcode/grokbuild/openclaw/pi/hermes/kimi/qoder/workbuddy/qwen/continue/cline/goose/zed/crush/dsh'
...
export function registerImportCommand(ctx, registryDir) {
  ...
  ctx.inject(['commands'], (cmdCtx) => {
    ...
    // /attach-workspaces：按 imports registry 回填 workspace。对早期未分组或
