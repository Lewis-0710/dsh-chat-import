...
// 当前运行版（DSH 0.1.5-rc.2）支持官方原生右侧栏（@deepseek-ai/dsh-client-ui-sidebar-
// right）：「导入会话」注册为右侧栏 tab 类型（guide 胶囊 + sidebar.right.pane.tab 面板
// 主体，点击 tab 在右栏内打开、对话区保留），footer 按钮点击改开该 tab；老版本无右侧
// 栏服务时自动回落既有行为（better-sidebar tab / 自绘 ShellPanel）。
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 官方原生右侧栏（@deepseek-ai/dsh-client-ui-sidebar-right，DSH ≥ 0.1.5-rc.1）的
    // 打开面：footer 按钮点击时经 ctx.get('sidebarRight').openTab(kind) 打开「导入会话」
    // tab（同一时刻展开右栏、对话区保留——接入侧栏接口而非覆盖主区）。闭包在 apply 里
    // 建立、点击时重证服务（缺失 / 抛错 / 无挂载会话都返回 false 让调用方回落，对齐
    // dsh-context 的 openContextSidebar 每次调用时重证的做法）；老版本恒为 null。
     ...
     *  与 better-sidebar tab 复用同一份内容）。面板的标题与关闭由右侧栏 tab 条自己呈现
     * （注册表 title 文本 + strip ✕），组件不需要再画一个头。 */
    ...
    function ImportButton({ wide }) {
      ...
      return React.createElement(React.Fragment, null,
        React.createElement("button", {
          ...
          onClick: () => {
            // 官方原生右侧栏可用 → 打开「导入会话」tab：tab 在右栏内打开、对话区
            // 保留（接入侧栏接口而非覆盖主区）；未开成 → better-sidebar tab；再退 →
            // 自绘 ShellPanel。seed 形态按服务版本给（见 importTabSeed）。
            // openNativeImportTab 每次点击重证 sidebarRight 服务（HMR 后服务可重挂）。
            if (openNativeImportTab && openNativeImportTab()) return;
    ...
    function apply(ctx) {
      ...
      // 官方原生右侧栏（@deepseek-ai/dsh-client-ui-sidebar-right，DSH ≥ 0.1.5-rc.1）的
      // tab 注册服务 sidebarRightTabs：deferred inject（不写进硬 inject，老版本回调
      // 不触发、fiber 不挂起——对齐 dsh-context 的 watchSidebarContextTab）。注册 tab
      // 类型（id/kind = chat-import，title 供 chip 与 guide 标题）、guide 胶囊（图标 +
      // 标题 + 描述）与面板主体槽 sidebar.right.pane.tab（session 作用域、keyed by id）。
      // 注册失败（id/kind 被占用等）只让右侧栏没有该 tab、其它入口照常（全保护注册，
      // 参照 dsh-context sidebar.ts 的 disposers + try/catch 收口）。注销器统一交给
      // inject 回调的返回：插件停止 / HMR 时右侧栏自动移除该 tab 类型与面板。
      if (typeof ctx.inject === "function") {
        ctx.inject(["sidebarRightTabs"], (sctx) => {
          const tabs = sctx && sctx.sidebarRightTabs;
          if (!tabs || typeof tabs.register !== "function") return;
          const disposers = [];
          const disposeAll = () => { for (const dispose of disposers) if (typeof dispose === "function") dispose(); };
          ...
          try {
            const disposeTab = tabs.register({
              id: IMPORT_TAB_TYPE,
            ...
            if (typeof disposeBody === "function") disposers.push(disposeBody);
          } catch (err) {
            // 吞掉的是「右侧栏 tab 本身」：id/kind 冲突等失败只影响该 tab，其它入口
            //（footer / better-sidebar / 自绘面板）不受影响；已注册的部分一并回滚。
            console.warn("[dsh-chat-import] 原生右侧栏 tab 注册失败（回落既有入口）：" + String((err && err.message) || err));
            disposeAll();
            return undefined;
          }
          return disposeAll;
      ...
      // footer 按钮的右栏打开面：点击时重证 sidebarRight 服务——0.1.5-rc.1 前的老版本
      // 没有该服务返回 false（走 better-sidebar / 自绘面板回落）；无挂载会话、服务被
      // 换掉（HMR）等失败同样 false，绝不让一次点击把浏览器打崩（参照 dsh-context 的
      // openContextSidebar；空 catch 吞掉的就是「本次打开」这一下，下一击重新尝试）。
      openNativeImportTab = () => {
        try {
