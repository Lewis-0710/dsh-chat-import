...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const SOURCES = [
      ...
      "opencode", "mimocode", "kilocode", "zcode", "grokbuild", "openclaw", "pi", "hermes", "kimi", "qoder", "workbuddy", "qwen", "dsh",
    ];
    // discovery format 短名 → 客户端来源 id（构建 /api-import/import 的 items）。
    const FORMAT_SOURCE = {
      claude: "claude-code", codex: "codex", chatgpt: "chatgpt", cursor: "cursor",
      gemini: "gemini", antigravity: "antigravity", reasonix: "reasonix", opencode: "opencode", mimocode: "mimocode", kilocode: "kilocode", zcode: "zcode",
      grokbuild: "grokbuild", openclaw: "openclaw", pi: "pi", hermes: "hermes",
      kimi: "kimi", qoder: "qoder", workbuddy: "workbuddy", qwen: "qwen", dsh: "dsh",
    };
    // 来源展示名（产品名不翻译）
    const SOURCE_LABELS = {
    ...
    function ImportTabComponent(props) {
      return React.createElement(ImportTabContent, {
        onClose: () => { if (betterSidebarService) betterSidebarService.closeTab(props && props.tab ? props.tab.id : IMPORT_TAB_TYPE); },
      });
    }
      ...
      const doImport = async (items, { replace = false } = {}) => {
        ...
        try {
          const resp = await fetch("/api-import/import", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ items, replace: replace === true }),
          });
          const data = await readJson(resp);
          if (data && data.ok === true) {
