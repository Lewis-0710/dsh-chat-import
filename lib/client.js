...
// 客户端 ModuleLoader 注入）。「导入会话」窗口接入官方**原生右侧栏**（hard 依赖：
// peerDependencies 把 dsh 抬到 ≥ 0.1.5-rc.1，inject 声明 sidebarRightTabs）——注册
// 右侧栏 tab 类型（guide 胶囊 + sidebar.right.pane.tab 面板主体，tab 在右栏内打开、
// 对话区保留），左侧栏底部「导入会话」按钮点击即开该 tab。无回落链：better-sidebar
// 集成与自绘 ShellPanel 浮层已随依赖抬升一并删除（REQ-84）。
