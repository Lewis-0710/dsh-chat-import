...
const emptyConfig = () => ({
  ...
  inbound: { enabled: false, formats: [...SYNC_FORMATS], excludeDirs: [] },
  outbound: { enabled: false, targets: [...SYNC_FORMATS], roots: {}, excludeDirs: [] },
  // watch 懒检查：面板打开时按 mtime 门控续写已导入源（无常驻监听）
  watch: { enabled: false },
...
// 目录键归一：`\`→`/`、折叠重复斜杠、去首尾空白与尾部斜杠（根 '/' 保留）。供排除目录
// 配置落盘与 sync-loop 匹配共用（同口径比较，避免分隔符/尾斜杠导致漏匹配）。
...
function normalizeExcludeDirs(list) {
  const seen = new Set()
  const out = []
  for (const d of (Array.isArray(list) ? list : [])) {
    const norm = normalizeDirPath(d)
...
function normalizeConfig(raw) {
  ...
  return {
    ...
    inbound: {
      ...
      excludeDirs: normalizeExcludeDirs(inbound.excludeDirs),
    },
    outbound: {
      ...
      excludeDirs: normalizeExcludeDirs(outbound.excludeDirs),
    },
