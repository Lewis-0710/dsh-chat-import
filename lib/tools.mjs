...
// （全部面板来源 + 本地 JSONL）由 makeImportChatTool 收敛为单一 import_chat 分发器；
...
import {
  ...
  convertContinueJson, readContinueIndex, convertClineJson, convertGooseJson, convertZedJson, convertCrushJson,
...
import { importGooseFile, importGooseDirectory } from './sources/goose.mjs'
import { importZedFile, importZedDirectory } from './sources/zed.mjs'
import { importCodexFile, importCodexDirectory, previewCodexFile, previewCodexDirectory } from './sources/codex.mjs'
import { importCrushFile, importCrushDirectory, crushDeriveArgs } from './sources/crush.mjs'
...
import { importOpencodeFile, importOpencodeDirectory } from './sources/opencode.mjs'
import { importMimocodeFile, importMimocodeDirectory } from './sources/mimocode.mjs'
import { importTeleagentFile, importTeleagentDirectory } from './sources/teleagent.mjs'
import { importKilocodeFile, importKilocodeDirectory } from './sources/kilocode.mjs'
import { importZcodeFile, importZcodeDirectory } from './sources/zcode.mjs'
...
import {
  ...
  previewTeleagentFile, previewTeleagentDirectory,
  previewKilocodeFile, previewKilocodeDirectory,
  previewZcodeFile, previewZcodeDirectory,
  previewGooseFile, previewGooseDirectory,
  previewZedFile, previewZedDirectory,
  previewCrushFile, previewCrushDirectory,
...
export function registerTools(ctx, registryDir) {
  ...
  const IMPORT_SOURCES = [
    ...
    { format: 'codex', sourceLabel: 'Codex/ChatGPT',
      io: {
        file: (c, t, a) => importCodexFile(c, t, a, { registryDir, runDecision, markTrimmedSource }),
        dir: (c, d, a) => importCodexDirectory(c, d, a, { registryDir, runDecision, markTrimmedSource }),
    ...
    // 自动定位 threads.db（<dir>/threads.db 或 <dir>/threads/threads.db）。
    {
      ...
      io: {
        file: (c, t, a) => importZedFile(c, t, a, { registryDir, runDecision, markTrimmedSource }),
        dir: (c, d, a) => importZedDirectory(c, d, a, { registryDir, runDecision, markTrimmedSource }),
    ...
    // projects.json 反查（DB 里没有 cwd 列）。
    {
      ...
      io: {
        file: (c, t, a) => importCrushFile(c, t, a, { registryDir, runDecision, markTrimmedSource }),
        dir: (c, d, a) => importCrushDirectory(c, d, a, { registryDir, runDecision, markTrimmedSource }),
  ...
  // 反向导出：export_claude / export_codex / export_kimi 三合一为
  ...
  definitions.push(defineTool({
    ...
    description:
      ...
      'JSONL; kimi = Kimi wire.jsonl; opencode = JSON for `opencode import <file>`. claude records ' +
      'the export mapping consumed by sync_to_claude target:"copy". dryRun serializes without ' +
      'writing. Returns target path, counts, degradations.',
    parameters: {
      format: {
        type: 'string',
        required: true,
    ...
    output: {
      schema: {
        ...
        properties: {
          ...
          toolCalls: { type: 'integer' },
          toolResults: { type: 'integer' },
          // 降级逐条报告（三类导出共享）：有损项（孤儿结果/注入跳过/附件跳过）
          degradations: {
            type: 'array',
            items: {
              type: 'object',
  ...
  // 反向同步（双向同步桥 B 第一步）：把 DSH 会话新增轮次增量写回 Claude Code
