...
// 各源格式的 `convertXxx(raw, args)`（见同目录 convert/*.mjs，现共 19 种：
// claude/codex/chatgpt/cursor/gemini/reasonix/opencode/mimocode/kilocode/zcode/grokbuild/
// openclaw/hermes/pi/kimi/qoder/workbuddy/dsh/local-jsonl）把原始 transcript 文本
...
// 'plugin'，plugin='chat-import'）钉在首个 turn 之前；开关开启时声明后附原始系统
// 提示词。正文按 dsh 惯例包 <system-reminder> 信封（见 contextInjectionText）。
// 声明告知模型：迁移到 DSH 后工具/权限/指令以 DSH 当前会话为准，避免沿用
// 源工具系统提示词里的旧工具名/旧命令（这些在 DSH 里不可用/不同）。
export function synthesizeSession({ meta, turns, title, provider, model, skipped, records, skippedLines = [], secrets = [], permissionCount = 0, systemPrompt }) {
  ...
  // 导入归属（REQ-32）只落 imports registry（sourcePath → dshId 反查即得），不写
  // 日志事件：dsh ≥ 0.1.2-alpha 的读取路径 fail-closed（KNOWN_SESSION_EVENT_TYPES
  // 白名单 + envelope 键白名单），宿主词汇表外的自产标记事件会让整份日志被拒载
  //（issue #34）。req33/req32 的标记读取按 registry 优先、旧日志标记兜底。

  // 环境变更声明（总是注入）：作为「上下文注入」的 user/message 钉在首个 turn 之前。
  // source.kind='plugin' 让 UI 折叠显示为「上下文注入 · chat-import」；正文前置环境
  // 变更声明（模型看到的是迁移后的新环境，旧工具名/命令不再适用）。开关开启时各源把
  // 原始系统提示词收集进 systemPrompt，由 contextInjectionText 附在声明之后。
  if (turns.length > 0) {
    push('user/message', {
...
// 等源事件，不校验）。指向集合外的引用合法（append 尾片的跨轮引用指向前段已导入
// 事件，此处无法验证）——不报。
...
export const SESSION_EVENT_TYPES = [
  'session/imported', // 本插件自产标记（宿主词汇表外）
  'session/title',
  'turn/start', 'turn/end',
  'step/start', 'step/end',
