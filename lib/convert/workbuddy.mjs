...
// 失败重发 step 清洗（ghost retry 去重，claude.mjs 同款）：一轮工具调用没等到结果而
// 中止时，源会在紧随的下一步用同一个 callId 原样重发。两条都保留会让 DSH 日志出现
// 重复 callId 的 tool/call，会话折叠器对同一 id 只允许一次 start（硬异常），首个
// 重复处之后的整段轨迹被吞掉。相邻两步、前一步只含 tool-call 块、无任何结果、且
// 全部 callId 在后一步同名同参重发 → 判定前一步为失败重发，整步丢弃（结果按 callId
// 配对到重发步，丢弃不丢内容）。已知边界：同一步内重复 callId、非相邻重发不处理。
...
export function convertWorkbuddyJsonl(raw, args = {}) {
  ...
  // 失败重发 step 清洗（见 dropDeadRetrySteps 注释），合成事件前执行
  ...
  // 标题兜底：WorkBuddy 无显式标题源 → 首问兜底（只回填 title，不钉事件）
