...
// 摘要作 reasoning 块前置（opencode/zcode/pi/claude 同款语义），避免把源已
...
export function convertKimiWire(raw, args = {}) {
  ...
  // 导入会话撑爆预算（被动裁剪保的是「导入预算的锚点+尾部」，并非「源模型
  ...
  if (compactionCut) {
    ...
    // 摘要 → reasoning 块前置到首个保留步骤（claude compacted 同款）。摘要文本
  ...
  // compacted：压缩截断已生效（claude compacted 报告同款，imports.mjs 白名单键）
  return {
    ...syn,
    title: finalTitle,
    droppedToolResults,
