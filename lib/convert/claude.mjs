...
export function convertClaudeJsonl(raw, args = {}) {
  ...
  // compacted 摘要导入：只导「最后一次压缩摘要 + 尾部」。Claude Code 的
  ...
  for (const rec of recs) {
    ...
    // custom-title（优先于 ai-title）：Claude 的会话标题载体是 summary 记录的
  ...
  // compacted：摘要 reasoning 块前置到首个保留轮的 assistant 步骤（opencode
  ...
  // 标题选取：custom-title > ai-title > 首问兜底。显式标题（custom/ai，归一后
