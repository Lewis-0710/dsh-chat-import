// lib/convert/antigravity.mjs — Antigravity 会话转录 → DSH 会话（纯函数）
//
// Antigravity 是 Gemini CLI 的后续产品，但与 Gemini CLI 共享 ~/.gemini 前缀、
// 存储格式完全不同（Gemini CLI 是 <slot>/chats/session-*.json，本工具是每会话一份
// 独立存储），因此独立成源而非复用 gemini.mjs。
//
// 存储：~/.gemini/antigravity/（2.0 / IDE）与 ~/.gemini/antigravity-cli/（旧 CLI）
// 同构，另有个别 IDE 发行用 ~/.gemini/antigravity-ide/：
//   conversations/<conversationId>.db | .pb   每会话命中记录（正文为 protobuf/SQLite）
...
// 跨平台绝对路径判据：转换层读的 transcript 可能来自另一个平台（Windows 路径在 Linux 上
// 导入、反之亦然），所以不能用 node:path 的 isAbsolute（它按当前平台判）。宿主 header 只收
// 绝对路径（prepareHostMeta 会把非绝对 cwd 剔除），判据太窄会让导入会话丢掉工作区归属，
// 后续 attachToWorkspace 只能回退源目录、再被宿主以「header 无 cwd」拒掉。
const isAbsoluteCwd = (p) => typeof p === 'string' && p !== ''
...
export function convertAntigravityJsonl(raw, args = {}) {
  ...
  if (isAbsoluteCwd(cwd)) meta.cwd = cwd
