...
//   reasoning     推理内容是否记录（chatgpt/cursor 无推理；codex 只有 summary 可读、密文不可读）
//   cwd           工作目录是否记录（chatgpt 是聊天无 cwd；grokbuild 无）
//   branches      会话内分支是否保留（chatgpt mapping DAG / pi 树）
//   attachments   附件（图片等非文本块）是否记录
//   compacted     压缩摘要是否感知（opencode/zcode/pi/kimi 有 compaction 还原）
export const SOURCE_CAPABILITIES = {
  ...
  // codex：reasoning 记录里 summary 块数组是可读文本（密文 encrypted_content 不读也不搬）
  codex: { toolResults: true, reasoning: true, cwd: true, branches: false, attachments: true, compacted: false },
  ...
  antigravity: { toolResults: true, reasoning: true, cwd: true, branches: false, attachments: false, compacted: false },
  ...
  // teleagent：opencode 派生（schema 同构，issue #60），能力矩阵与 opencode 相同；
  // 样本中的 compaction part 只有 {auto:true}、无 tail_start_id → 不触发裁剪，全量导入
  teleagent: { toolResults: true, reasoning: true, cwd: true, branches: false, attachments: true, compacted: true },
  ...
  kimi: { toolResults: true, reasoning: true, cwd: true, branches: false, attachments: false, compacted: true },
  qoder: { toolResults: true, reasoning: true, cwd: true, branches: false, attachments: false, compacted: false },
  // continue：contextItems 引用的图片不落会话（attachments false）；conversationSummary
  // 是压缩摘要标记，history 本身不裁剪（compacted true，导入保留全文 + 摘要 reasoning 块）
  continue: { toolResults: true, reasoning: true, cwd: true, branches: false, attachments: false, compacted: true },
  // cline：image/file/media 附件块与 redacted_thinking 密文不读（attachments false）；
  // 压缩侧车 <id>.compaction.json 存在但压缩不改写主转写（compacted true）
  cline: { toolResults: true, reasoning: true, cwd: true, branches: false, attachments: false, compacted: true },
  // goose：image/document 块与 redactedThinking 密文不读（attachments false）；
  // sessions/messages 两表没有会话内分支概念（branches false）
  goose: { toolResults: true, reasoning: true, cwd: true, branches: false, attachments: false, compacted: false },
  // zed：Mention/Image 块与 RedactedThinking 不读（attachments false）；压缩是会话内
  // Compaction 记录（Summary 挂 reasoning 块还原边界），但历史由 Zed 自己裁剪 → compacted true
  zed: { toolResults: true, reasoning: true, cwd: true, branches: false, attachments: false, compacted: true },
  // crush：image_url/binary/shell_command 块不读（attachments false）；自动摘要消息挂
  // reasoning 块还原压缩边界（compacted true）；子会话有 parent_session_id 但默认不导入
  crush: { toolResults: true, reasoning: true, cwd: true, branches: false, attachments: false, compacted: true },
