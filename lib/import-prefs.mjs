...
export function resolvePrefsBinding(ctx, settingsOverride) {
  ...
  if (settings) prefsBindings.set(ctx, binding)
  return binding
}
...
// forms 路径订阅 settings/document-updated（0.1.7 无 scope.watch）；legacy 路径用
// scope.watch。settings 缺席时回调永不触发，工具保持 registerTools 的默认注入态。
export function registerImportPrefs(ctx, onInjectToolsChange) {
  ctx.inject(['settings'], (sctx) => {
    try {
      const binding = resolvePrefsBinding(ctx, sctx && sctx.settings)
      if (binding.mode === 'none') return
      const settings = binding.settings
      if (binding.mode === 'forms') {
        // 页面策略：本插件自带侧栏面板，不需要宿主再生成一个自动设置页。经 sctx.effect
        // 登记（对齐宿主插件 child.effect(() => child.settings.configure(...)) 的姿势），
        // configure 的 disposer 随插件卸载释放——直接调用会把策略留在服务里，重载时抛
        // "already configured"。
        if (ctx.fiber && typeof settings.configure === 'function') {
          try {
        ...
        try {
          // { global: true }：事件由设置服务在自己的 owner 上下文发出，跨 fiber 也要收到
          //（对齐宿主 dsh-api-remotes 对该事件的转发口径）。
          sctx.on('settings/document-updated', (ns) => {
            if (ns === binding.ns) onInjectToolsChange(current())
          }, { global: true })
        } catch { /* 事件不可用：实时对账退化为下次读取，不影响导入 */ }
        return
      }
      // legacy（0.1.5）：插件自持命名空间
