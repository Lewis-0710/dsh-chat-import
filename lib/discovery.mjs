// lib/discovery.mjs — REQ-25/REQ-40 会话发现索引：全部来源格式统一 discover + 30s TTL 扫描缓存
//
...
import { isDshSessionFile } from './dsh.mjs'
...
import { codexThreadIdFromName, codexNameTimestamp } from './codex.mjs'
...
export const FORMATS = [
  ...
  'zcode', 'grokbuild', 'openclaw', 'pi', 'hermes', 'kimi', 'kilocode', 'qoder', 'chatgpt', 'workbuddy', 'qwen', 'continue', 'cline', 'goose', 'zed', 'crush', 'teleagent', 'dsh',
]
...
export function defaultRoots({ home = homedir() } = {}) {
  ...
  return {
    ...
    // antigravity：Antigravity CLI 复用 ~/.gemini 前缀但自有存储根（与 gemini 并列）
...
// v2：makeEntry 新增 contextTokens——旧书签条目缺该字段，读命中会退回「—」，故 bump 版本
// 整体失效旧缓存（下一次扫描重读并回填），避免「改了代码但旧缓存不刷新」的假阴性。
...
// 上下文 token 数（「会话规模」指标，替代 messageCount 的 null 展示）：只读转录尾部取
...
// 发现层 fallback 到源文件目录）。
function makeEntry({ format, sessionId, title, project, createdAt, lastActiveAt, messageCount, contextTokens, sourcePath, cwd }) {
...
export function layoutProject(sourcePath, format) {
  ...
  switch (format) {
    ...
    case 'antigravity': {
      ...
      // 无项目分目录概念（每会话一个 id 目录）→ 固定源标签，工作区由 cwd 归组。
      const m = p.match(/\/antigravity-cli\/brain\/([^/]+)\//)
  ...
  for (const file of files) {
    ...
    const entries = await probeSource(bm, 'claude', file.path, fp, async () => {
      ...
      return [makeEntry({
        ...
        createdAt, lastActiveAt: st.mtimeMs, messageCount: null, contextTokens, sourcePath: file.path, cwd,
      })]
    })
    out.push(...entries)
...
async function scanCodex(host, target, bm, emit) {
  ...
  for (const [threadId, pages] of groups) {
    ...
    // 指纹取链的复合（size 求和 + mtime 最大）：新增一页即变化，触发 REQ-24 append
    let sizeSum = 0
    let mtimeMax = 0
    ...
    const entries = await probeSource(bm, 'codex', root.path, { mtimeMs: mtimeMax, sizeBytes: sizeSum }, async () => [makeEntry({
      ...
      createdAt, lastActiveAt: mtimeMax || undefined, messageCount: null,
  ...
  for (const file of files) {
    ...
    const entries = await probeSource(bm, 'cursor', file.path, fp, async () => {
      ...
      return [makeEntry({
        ...
        createdAt, lastActiveAt, messageCount: null, sourcePath: file.path, cwd,
      })]
    }, host)
    out.push(...entries)
    await emitEach(emit, entries)
...
// antigravity：~/.gemini/antigravity-cli/ 的每会话一目录布局（Antigravity CLI 是
// Gemini CLI 的后续产品，共用 ~/.gemini 前缀但存储形态完全不同，故独立成源）：
//   conversations/<id>.db                                命中记录（正文是 protobuf，不读）
//   brain/<id>/.system_generated/logs/transcript.jsonl   明文逐行 JSON（发现与导入都读它）
//   annotations/<id>.pbtxt                               标题（protobuf 文本格式）
// 条目按 conversations/*.db 枚举（会话即该文件），sourcePath 指向 transcript.jsonl——
// 它才是导入输入；缺 transcript 的会话无正文可导，不产出条目。
async function scanAntigravity(host, target, bm, emit) {
  ...
  const files = []
  await walkFiles(host, convDir, files, (name) => /\.db$/i.test(name))
  const out = []
  for (const file of files) {
    const id = basenameOf(file.name).replace(/\.db$/i, '')
    if (!id) continue
    ...
    const entries = await probeSource(bm, 'antigravity', transcriptPath, fp, async () => {
      // 只读头部抽元数据（大转录不整读）：USER_INPUT/PLANNER_RESPONSE 计数即会话规模。
      ...
      for (const rec of recs) {
        ...
        if (rec.type === 'USER_INPUT' || rec.type === 'PLANNER_RESPONSE') msgCount++
  ...
  for (const file of files) {
    ...
    const entries = await probeSource(bm, 'qwen', file.path, fp, async () => {
      ...
      return [makeEntry({
        ...
        createdAt, lastActiveAt: st.mtimeMs, messageCount: null, sourcePath: file.path, cwd: wsDir,
      })]
    })
    for (const e of entries) {
  ...
  for (const dir of dirs) {
    ...
    const entries = await probeSource(bm, 'kimi', dir, fp, async () => {
      ...
      return [makeEntry({
        ...
        createdAt, lastActiveAt: wst.mtimeMs, messageCount: null, contextTokens, sourcePath: dir, cwd,
      })]
    })
    out.push(...entries)
    await emitEach(emit, entries)
...
// readDshText 全量解压、title/messageCount 以导入结果为准），首次扫描不再分钟级；
// 之后条目随 mtime/size 书签跳过。
const DSH_ZSTD_SCAN_MAX_BYTES = 256 * 1024

async function scanDsh(host, target, bm, emit) {
  const files = []
  await walkFiles(host, target, files, (name) => isDshSessionFile(name))
  const out = []
  for (const file of files) {
    // 导入产物目录（import-<id>）不再当源扫出——重复列出且重导它们等于递归自我导入
    const parts = file.path.split(/[\\/]/)
    const dirName = parts[parts.length - 2]
    if (typeof dirName === 'string' && dirName.startsWith('import-')) continue
    const st = await host.stat(file.path)
    if (!st) continue
    const fp = { mtimeMs: st.mtimeMs, sizeBytes: st.size }
    ...
    if (/\.zstd$/i.test(file.path) && st.size > DSH_ZSTD_SCAN_MAX_BYTES) {
      const entries = [makeEntry({
        format: 'dsh', sessionId: dirName, title: '',
        project: layoutProject(file.path, 'dsh'),
        createdAt: st.mtimeMs, lastActiveAt: st.mtimeMs,
        messageCount: 0, sourcePath: file.path,
      })]
      out.push(...entries)
      await emitEach(emit, entries)
      continue
    ...
    const entries = await probeSource(bm, 'dsh', file.path, fp, async () => {
      ...
      if (/\.zstd$/i.test(file.path)) {
        try {
          text = Buffer.from(decompress(await fread(file.path))).toString('utf8')
        } catch {
          return []
        }
      } else {
...
async function scanContinue(host, target, bm, emit) {
  const files = []
  await walkFiles(host, target, files, (name) => /\.json$/i.test(name))
  // 索引优先：一次读入 sessionId → { title, createdAt, cwd, messageCount }
  let index = new Map()
  for (const file of files) {
  ...
  for (const file of files) {
    ...
    const entries = await probeSource(bm, 'continue', file.path, fp, async () => {
      ...
      let messageCount = known ? known.messageCount : null
      ...
      if (!known) {
        ...
        cwd = typeof session.workspaceDirectory === 'string' && session.workspaceDirectory
          ...
          : null
        messageCount = session.history.filter((it) => it && it.message
          && (it.message.role === 'user' || it.message.role === 'assistant')).length
      }
      return [makeEntry({
        ...
        messageCount, sourcePath: file.path, cwd,
      })]
    })
    out.push(...entries)
    await emitEach(emit, entries)
    ...
    const entries = await probeSource(bm, 'cline', dbPath, fp, async () => {
      ...
      for (const s of sessions) {
        ...
        out.push(makeEntry({
          ...
          messageCount: null, sourcePath: path, cwd: s.cwd,
        }))
      }
      return out
    })
  ...
  for (const file of files) {
    ...
    const entries = await probeSource(bm, 'cline', file.path, fp, async () => {
      ...
      return [makeEntry({
        ...
        messageCount: null, sourcePath: file.path, cwd: (man && man.cwd) || null,
      })]
    })
    out.push(...entries)
    await emitEach(emit, entries)
...
// lib/goose.mjs）。旧版 sessions/*.jsonl **不扫**：上游只在首次建库时全量迁移且不删旧文件，
// 扫它们会与库里的同一批会话重复导入。
...
// lib/zed.mjs）。子代理线程（parent_id 非空）由读取层过滤。
  ...
  for (const dbPath of dbPaths) {
    ...
    const entries = await probeSource(bm, 'crush', dbPath, fp, async () => {
      ...
      return sessions.map((s) => makeEntry({
        ...
        messageCount: s.messageCount, sourcePath: dbPath, cwd: projectPath,
      }))
    })
    out.push(...entries)
    await emitEach(emit, entries)
...
export async function scanFormat(host, format, target, bm, emit) {
  ...
  if (!fn) return []
  try {
    return await fn(host, target, bm, emit)
  } catch {
...
function fileFormatsForPath(path) {
  ...
  if (/(^|[\\/])sessions[\\/]/.test(lower) && isDshSessionFile(base)) return ['dsh']
  ...
  if (/\.json$/i.test(lower)) {
    ...
    if (/\.messages\.json$/i.test(lower)) return ['cline']
...
export async function discoverSessions({ path, format, query, home, host, imports, cache, cacheDir, archivedIds, onEntry } = {}) {
  ...
  const emitMapped = typeof onEntry === 'function'
    ? async (entry) => {
      const mapped = {
        ...entry,
        importStatus: resolveImportStatus(reg, entry.sourcePath, entry.sessionId, archivedIds),
  ...
  const sessions = await Promise.all(all.map(async (e) => ({
    ...e,
