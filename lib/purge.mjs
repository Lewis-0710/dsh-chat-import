...
export async function listImportHistory(ctx, registryDir) {
  ...
  for (const [sourcePath, raw] of Object.entries(registry.imports || {})) {
    ...
    if (record.kind === 'multi') {
      for (const table of ['conversations', 'sessions']) {
        ...
        for (const [sourceSessionId, sub] of Object.entries(subs)) {
          ...
          if (canReadSessionEvents(sp)) {
            const info = await readSessionLog(ctx, sub.dshId)
            if (info) {
      ...
      if (canReadSessionEvents(sp)) {
        const info = await readSessionLog(ctx, record.dshId)
        if (info) {
          const title = sessionTitleFromEvents(info.events)
          if (title) item.title = title
...
async function pruneRegistryAfterDelete(registryDir, sourcePath, sessionId) {
  ...
  if (!record) {
    ...
    await rememberIgnore(registryDir, { key: sourcePath, reason: 'retracted', dshId: sessionId })
    return
  }
  if (record.kind === 'multi') {
    let anyLeft = false
    let scopedKey = sourcePath
    for (const table of ['conversations', 'sessions']) {
      const subs = record[table] && typeof record[table] === 'object' ? record[table] : null
      if (!subs) continue
      for (const [key, sub] of Object.entries(subs)) {
        if (sub && sub.dshId === sessionId) {
          scopedKey = sourceIgnoreKey(sourcePath, table, key)
          delete subs[key]
        }
      }
    ...
    // 删除（purge）是用户显式动作：打永久墓碑，源增长/重扫都不会把它带回来
    await rememberIgnore(registryDir, { key: scopedKey, reason: 'retracted', dshId: sessionId, sourcePath })
    return
  }
  await removeImport(registryDir, sourcePath)
  await rememberIgnore(registryDir, { key: sourcePath, reason: 'retracted', dshId: sessionId })
