...
// 以及 opencode / zcode / hermes / grokbuild / chatgpt / kimi 的 dry-run 预览
...
import {
  ...
  convertGooseJson, convertZedJson, convertCrushJson,
...
import {
  ...
  previewTranscript, previewDirectory, previewEntry, attachConversionDetails, runDecision, batchItem,
...
import { readMimocodeDb } from './sources/mimocode.mjs'
import { readKilocodeDb } from './sources/kilocode.mjs'
import { readTeleagentDb } from './sources/teleagent.mjs'
import { convertTeleagentJson } from './convert/index.mjs'
import { readZcodeDb, readZcodeTranscript, zcodeDefaultDbPath } from './sources/zcode.mjs'
import { readGooseDb } from './sources/goose.mjs'
import { readZedDb } from './sources/zed.mjs'
import { readCrushDb } from './sources/crush.mjs'

// ── ChatGPT 导出导入：单个 conversations.json 可能含多个会话，每个会话独立落盘
//（逐会话判增 append / 消失 missingFromSource；force=全量新副本）。
export async function importChatgptFile(ctx, target, args, { registryDir, persisted } = {}) {
  ...
  // branch 参数透传（main 默认 / all 全部分支会话）
...
// registry 的 sizeBytes/version 落复合值，短路径判定对双文件都有效。
...
export async function importGrokbuildSession(ctx, target, args, { registryDir, persisted } = {}) {
  ...
  if (known && args.force !== true && args.replace !== true && !isSessionIdChange(args, known.dshId)) {
    ...
    // 预算变化（文件未变）→ 跳过并报告（同 argsChanged 语义）
  ...
  return attachConversionDetails(out, await runDecision(ctx, decision, registryDir, sourcePath, persistedSet, { workspaceMode: args.workspaceMode, workspaceDir: args.workspaceDir }))
}
...
// DB 级 version/size 短路径检测；逐会话判增 append / 会话消失 missingFromSource。
...
export async function importHermesDbFile(ctx, target, args, { registryDir, persisted, sessions } = {}) {
  ...
  const preSkipped = []
  // lineage 过滤：lineage:'tail' 只导叶子链尾（非任何会话的父会话）；
...
// opencode 预览：lib/sources/opencode.mjs 的 importOpencodeFile 编排（registry / decideMulti /
...
// teleagent 预览：opencode 派生，同构 SQLite 只读重演（readTeleagentDb 已兼容无
// model 列 schema），provider 标签为 teleagent。
export async function previewTeleagentFile(ctx, target, args) {
  const path = target.displayPath || ctx.fs.processPath(target)
  const sessions = readTeleagentDb(path, { fullHistory: args.fullHistory === true })
  const wanted = Array.isArray(args.sessionIds) && args.sessionIds.length > 0 ? new Set(args.sessionIds) : null
  const results = []
  for (const s of sessions) {
    if (wanted && !wanted.has(s.id)) continue
    const out = markTrimmedSource(convertTeleagentJson(JSON.stringify(s), { ...args, sourcePath: path }), args)
    if (!out.meta || (out.turns.length === 0 && out.events.length === 0)) {
      results.push({ path, skipped: 1, skipReason: 'no user turns (session ' + s.id + ')' })
      continue
    }
...
export async function previewTeleagentDirectory(ctx, dirTarget, args) {
  const dirPath = dirTarget.displayPath || ctx.fs.processPath(dirTarget)
  const dbTarget = await ctx.fs.resolve(join(dirPath, 'teleagent.db'))
  return previewTeleagentFile(ctx, dbTarget, args)
}
...
//（lib/sources/zcode.mjs 编排不可改，预览绕开 registry / decideMulti / 落盘）。
...
// crush 预览：项目内 crush.db 一库多会话，只读重演（绕开 registry / decideMulti / 落盘）。
...
export async function previewCrushDirectory(ctx, dirTarget, args) {
  ...
  for (const candidate of [join(dirPath, '.crush', 'crush.db'), join(dirPath, 'crush.db')]) {
    const info = await ctx.fs.stat(await ctx.fs.resolve(candidate))
    if (info && info.type === 'file') return previewCrushFile(ctx, await ctx.fs.resolve(candidate), args)
  }
  return previewCrushFile(ctx, await ctx.fs.resolve(join(dirPath, 'crush.db')), args)
}
