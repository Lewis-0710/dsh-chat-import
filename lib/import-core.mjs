...
// 轻量 cwd 可见性回退：cwd 在本地不存在/不可解析（realpath 拒绝——跨机器迁移
...
export async function attachToWorkspace(ctx, meta, sourcePath, options = {}) {
  ...
  for (const path of candidates) {
    // 沙箱防护：cwd = 用户主目录时 dsh 沙箱 ACL 拒绝（temp 在 workspace 内，
...
export function prepareHostEvents(events, sessionId, version = SESSION_FORMAT_VERSION) {
  const stripped = []
  ...
  return shapeMessageSources(shapeToolResults(safe, version), version)
}
...
// 自己的 kind（kind:'plugin:X'；system-prompt 生产者另有 'system-prompt' 映射），读路径对
// V4 日志直接拒绝 'plugin'（"format v4 message requires a producer-owned source kind"）。
// 导入自产的环境变更声明与 system head 都用 kind='plugin'，写 V4 时必须同步改写，否则宿主
// 读不回自己刚写下的日志。V3 保持原样（'plugin' 是 V3 的合法形状）。改写规则对齐宿主
// dsh-session-format-v3-to-v4 的 rewritePluginSource。
...
function shapeMessageSources(events, version) {
  ...
  const fixSource = (source, role) => {
    ...
    return { ...rest, kind: plugin === SYSTEM_PROMPT_PLUGIN && role === 'system' ? 'system-prompt' : 'plugin:' + plugin }
  }
...
async function appendHostEvents(ctx, targetId, events) {
  ...
  await ctx.sessionPersistence.append(targetId, prepareHostEvents(events, targetId, version))
}
...
async function createSession(ctx, meta, events) {
  ...
  const hostEvents = prepareHostEvents(events, hostMeta.id, hostVersion)
  const agents = ctx.get('agents')
  // 显式代次目标（面板「导入到 → DSH（V3/V4 会话格式）」）必须落成指定 generation：
  // agents.create 只按宿主当前代次种会话，所以有覆盖时直接走 sessionPersistence.create
  // （它按 header.version 选文件名）。
  if (agents && typeof agents.create === 'function' && formatVersionOverrides.get(ctx) === undefined) {
    let presetId
    try {
...
export function batchItem(path, single) {
  ...
  for (const k of ['skipReason', 'error', 'appendedTurns', 'appendedEvents', 'appendedSkipped', 'sourceShrunk', 'changedInPlace', 'argsChanged', 'budgetChanged', 'backfilled', 'droppedBoundaryResults', 'orphanToolResults', 'duplicateToolResults', 'forceImported', 'staleGhost', 'trimmed', 'skippedLines', 'secrets', 'permissionCount', 'walMerged', 'walRecords', 'compacted', 'replaced']) {
