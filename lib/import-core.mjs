...
// REQ-74 cwd 重映射：跨机器迁移时，把源机的 cwd 前缀映射到本机等价路径。
//
...
function cwdRemapRules(args) {
  ...
  return raw
    .filter((r) => r && typeof r.from === 'string' && r.from && typeof r.to === 'string' && r.to)
    .map((r) => ({ from: r.from.replace(CWD_TRAILING_SEP, ''), to: r.to.replace(CWD_TRAILING_SEP, '') }))
...
export function applyCwdRemap(out, args) {
  ...
  for (const { from, to } of rules) {
    ...
    const mapped = rest ? join(to, ...rest.split(/[\\/]+/)) : to
    out.meta.cwd = mapped
