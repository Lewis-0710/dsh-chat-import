...
- **导入面板图标选中态对比度** — 选中遮罩不再用强调色（会与勾选图标同色导致看不清），改为按强调色 HSV 明度选固定黑/白：V > 40（亮色）用黑遮罩，否则白遮罩；勾选环/勾改用强调色。
## [0.18.3] - 2026-09-18
...
- **数据库类批量来源的会话标题钉「来源 · 话题」（REQ-68，随新源一并修复）** — opencode / mimocode / kilocode / zcode 走 `decideMulti` 批量管线，此前只做预算裁剪（`markTrimmedSource`），不经过统一的标题钉化步骤，UI 会把会话标题回退成工作区目录名。现在这四条管线（以及新接入的 teleagent）都经 `finalizeConvertedSession` 收口：钉「来源 · 话题」标题，同时顺带获得与其他来源一致的 `cwdRemap` 口径。
...
- **Codex 分页链扫描改成按 thread 过滤，不再按全树文件数截断（#57 跟进）** — 0.17.1 的链解析沿用了「收集目录树下全部 `rollout-*.jsonl`（上限 512 个）」的旧遍历。`~/.codex/sessions` 累积到几百个会话后，这 512 个名额会先被**无关会话**的文件占满，某个 thread 的分页只收到一部分、甚至收不到用户指定的那一页，结果是**静默导入一条缺页的链**——正是 #57 要修的那类问题，而且收哪些页取决于 `readdir` 顺序，同一份数据两次导入可能不一致。现在遍历时就按文件名里的 thread id 过滤（不留无关文件），预算分两个口径：看过 **20 万个目录项**、本链 **2048 页**，任一触顶直接抛错并说明原因，宁可大声失败也不交付半截会话。副产品：只对真正属于该链的分页读文件头，链解析不再对上百个无关 rollout 各读 256 KB 头部；同一目录被多个搜索根覆盖时按路径去重。
  （这条修复曾在历史里以 `0.17.2` 的名义 bump 过版本号，但那一步只落到 commit，从未发布到 npm、也没有 tag 与 GitHub Release；因此它作为本版内容**首发**。npm 上没有 `0.17.2`。）
...
> 尚未覆盖：VS Code 扩展的 **legacy `tasks/`**（`api_conversation_history.json` + `ui_messages.json`，新 SDK 存储尚未迁移它，见 REQ-66）。
...
- **与内置「手机连接」入口同槽时按钮被挤成窄条 / 截断：占用者检测改按布局事实判定** — footer 槽是 nowrap 行，任何整宽条目进来都会把「导入会话」按钮挤窄；此前靠选择器白名单识别占用者，于是每出现一个新来源就得补一条（cordis 徽标 → 插件市场 launcher → usage-billing 计费卡 → cost-meter 费用卡 → 内置 `@agents-anywhere/dsh-bridge-next` 的「手机连接」），而 CSS-module 哈希类名跨构建不稳定，白名单永远追不上（#31 / #35 / #39 / #43 是同一类问题的四次复发）。现在改按**布局事实**判定，与「占用者是谁」无关：① 本插件的槽元素内容被裁（`scrollWidth > clientWidth`）——「被挤成窄条 / 文字截断」的直接症状，占用者即使已被 flex 压扁也必然留下这个痕迹（#43 实测占用者声明 `calc(100% + 4px)` 却被压到 185px / 容器 256px，纯几何判据会漏）；② 同槽存在渲染宽度≈容器宽的可见条目。半宽入口共享一行（#31 的预期行为）两个信号都不命中，行为不变。原白名单保留为历史兜底，**新增来源无需再补**。判定逻辑收敛为 `lib/footer-layout.mjs` 的纯函数并单测，`lib/client.js` 内联同款副本（bundle 不做构建）。
...
- **导入 DSH 自身会话：透传路径补齐宿主合规标记** — dsh 源按原样透传旧日志，其中的 surface 事件缺 `surfaceOp`、assistant/message 缺 `stream`、tool/result 的 source 是旧式 user-kind，任一条都会让整份种子被拒；透传前逐条归一，无法归一的 tool/result（两侧都没有 callId、无法关联任何调用）丢弃并计数上报 `droppedEvents`。同一处还修掉了 turns IR 读错 tool/result 位置的问题（DSH 把工具身份放在 `data.message.content[0].toolCallId` 与 `source.callId`，`data` 上并没有顶层 `toolCallId`，此前续写 IR 的工具结果恒为空）。
- **导入撞「会话已存在」不再被回退吞掉** — 宿主内存索引残留同名会话时（幽灵 id），`agents.create` 的 already-exists 错误此前会落进 `sessionPersistence` 回退，而回退在新宿主上必然失败并抛出格式错误，于是「另铸后缀新 id 重试」永远不触发、同一批幽灵目录每轮同步重复失败。现在 session-taken 错误原样上抛交给重铸逻辑，回退自己撞上 already-exists 时同样上抛。
- **适配 dsh 0.1.5 的句柄式会话 API** — 宿主的 `sessionPersistence` 换成 per-session 句柄：`list()` 返回 `{ header, revision, sizeBytes }` 元素（不再是 header 数组）、`readFrom` / `inspect` 移除（改 `open(id,'read')` + `handle.read()`）、`create` 返回写句柄且续写走 `handle.append()`（`append(id, events)` 移除）。插件此前按旧形状取值会让已持久化 id 集合全部变成 undefined——判重恒不命中，每次同步都当作新导入、另铸新 id 重复落盘。现在这层差异收敛在 `lib/imports.mjs` 的适配层，导入 / 导出 / 写回 / 撤回 / 校验 / 体检走同一套「列会话 / 读事件 / 建会话」语义，新旧宿主行为一致。
- **doctor 报告磁盘上宿主读不出的残留导入会话目录** — 升级前留下的旧格式 / 半成品 `import-*` 目录既不在 imports registry 也不在 `sessionPersistence` 里，却仍占用会话 id（重导时只能另铸带后缀的新副本）；doctor 现在只读列出它们并提示可手工清理，不写任何文件。
- **折叠（rail）态下「导入会话」浮层压住同槽「用量/余额」行** — `ImportButton` 的 fixed 浮层兜底只判定了「容器 nowrap + 存在整行占用者」，漏了 rail 态：rail 态刻意不注入 `flex-wrap`（#25/#31 的居中约束），而 56px 纵栏里同槽占用者（`@ychris12138/dsh-usage-stats` 的 `billing-trigger` 等）与本按钮同处一列，浮层按占用者 rect 定位后正好压在其「用量/余额」行上——宽态因会主动注入 wrap 而正常，#35 的占用者白名单修复只覆盖了宽态路径。现在 rail 态恒不浮层，走行内 36×36 圆钮分支，与同槽无占用者时同款。
